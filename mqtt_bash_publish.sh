...
until [ $# -eq 0 ];do
    case $1 in
	-a|--mqtt-ca) shift;p_ca=$1;;
	-C|--mqtt-cert) shift;p_cert=$1;;
	-k|--mqtt-key) shift;p_key=$1;;
...
user=$(get_config user 0)
password=$(get_config password 0)
qos=$(get_config qos 1)
log_file=$(get_config log_file 0)
action=$(get_config action 0)
ca=$(get_config ca 0)
cert=$(get_config cert 0)
key=$(get_config key 0)
...
params="" 
test -z "$user" || params="$params -u $user"
test -z "$password" || params="$params -P $password"
